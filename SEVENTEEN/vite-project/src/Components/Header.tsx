...
const Header:React.FC<HeaderProps> = ({ handleToggle, toggleMenu, products }) => {
    return (
        <div className='w-fit h-fit flex flex-col justify-end'>
            <button
                onClick={handleToggle}
                className='w-8 h-8 border-black border rounded-2xl cursor-pointer self-end'>
                {toggleMenu ? 'X' : '0'}
            </button>
            <div className={`p-4 flex flex-col gap-2 transition-all duration-200 rounded-2xl border-[0.5px] border-gray-400
